...
#  git log --pretty="%C(Yellow)%h  %C(reset)%ad (%C(Green)%cr%C(reset))%x09 %C(Cyan)%an: %C(reset)%s" --date=short $1...HEAD -n $2 && "-- components/$2"
if [ -n "$2" ]; then
...
export LDFLAGS="-L/opt/homebrew/opt/openssl@1.1/lib -L${HOME}/usr/local/Cellar/python@3.7/3.7.13_1/lib"
export WHEELHOUSE="/usr/local/share/pip-wheelhouse"
...
alias sdmy-dev="pnpm --filter sportsdirect-my dev"
alias cat="bat"
